...
const KATEGORIYALAR = [
  { id: "sotuv",     label: "Sotuv",      icon: "shopping-cart" as const, color: "#10B981" },
  { id: "xizmat",    label: "Xizmat",     icon: "tool"          as const, color: "#3B82F6" },
  { id: "qaytarildi",label: "Qaytarildi", icon: "rotate-ccw"    as const, color: "#F59E0B" },
...
export default function KirimScreen() {
  ...
  const [category, setCategory] = useState("sotuv");

  const { data: todayTxns = [], isLoading } = useQuery<Txn[]>({
    queryKey: ["kirim-today"],
    queryFn: () => apiReq<Txn[]>("/finance/transactions?period=today&type=income"),
    retry: false,
  });

  const { data: monthTxns = [] } = useQuery<Txn[]>({
    queryKey: ["kirim-month"],
    queryFn: () => apiReq<Txn[]>("/finance/transactions?period=month&type=income"),
    retry: false,
  });
  ...
  const katCfg = (id: string) => KATEGORIYALAR.find(k => k.id === id) ?? KATEGORIYALAR[3];
  ...
  return (
    <View style={[s.root, { backgroundColor: C.background }]}>
      ...
      <View style={[s.header, { paddingTop: topPadding + 10, borderBottomColor: C.border }]}>
        <View>
          <Text style={[s.headerTitle, { color: C.text }]}>Kirim</Text>
      ...
      >
        ...
        <View style={{ flexDirection: "row", gap: 10 }}>
          <View style={[s.sumCard, { flex: 1, backgroundColor: "#ECFDF5", borderColor: "#A7F3D0" }]}>
            <Feather name="sun" size={18} color="#10B981" />
            <Text style={[s.sumVal, { color: "#059669" }]}>{sum(todayTotal)}</Text>
            <Text style={[s.sumLbl, { color: "#10B981" }]}>Bugungi kirim</Text>
          </View>
          <View style={[s.sumCard, { flex: 1, backgroundColor: "#F0FDF4", borderColor: "#BBF7D0" }]}>
            <Feather name="calendar" size={18} color="#22C55E" />
            <Text style={[s.sumVal, { color: "#16A34A" }]}>{sum(monthTotal)}</Text>
            <Text style={[s.sumLbl, { color: "#22C55E" }]}>Bu oylik kirim</Text>
          </View>
        </View>
        ...
        {/* Bugungi kirimlar */}
        <View style={[s.section, { backgroundColor: C.card, borderColor: C.border }]}>
          <Text style={[s.sectionTitle, { color: C.textSecondary }]}>Bugungi kirimlar</Text>
          ...
          {!isLoading && todayTxns.length === 0 && (
            <View style={{ alignItems: "center", paddingVertical: 24, gap: 8 }}>
              ...
              <Text style={[s.emptyTxt, { color: C.textSecondary }]}>Bugun kirim yo'q</Text>
            </View>
          )}
          <View style={{ gap: 6 }}>
            {todayTxns.map(t => {
              ...
              return (
                <View key={t.id} style={[s.txnRow, { backgroundColor: C.surface, borderColor: C.border }]}>
                  <View style={[s.katIcon, { backgroundColor: kat.color + "18" }]}>
